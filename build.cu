...
public:
    ...
    bool getBatch(void *bindings[], const char *names[], int nbBindings) noexcept override
    {
        ...
        for (int i = 0; i < mBatchSize; i++)
        {
            ...
            process_input_gpu(img, mDeviceBatchData + i * offset, kInputW, kInputH);
        }
...
int main(int argc, char **argv)
{
    if (argc < 6)
    {
        std::cerr << "用法: ./build [onnx_file_path] [use_int8] [calib_dir] [calib_list_file] [engine_path]" << std::endl;
        return -1;
    }
    ...
    {
        sample::gLogInfo << "设备不支持或不使用int8." << std::endl;
